...
class ChainingHashTable():
    ...
    ***** DON'T worry you will fix this later:)
    ...
    def __contains__(self, item):
        ...
        i = self._hash(item) % self.n_slots
        list_at_index = self._data[i]
        found = False
        for element in self._data[i]:
            if element == item:
                found = True
                return True 
        if not found:
...
class LinearHashTable():
    ...
    def store(self, item):
        ...
        # ***********************************************************
        # ---start student section---
        new_hash = self._hash(item) % self.n_slots
        while self._data[new_hash]:
            new_hash = (new_hash + 1) % self.n_slots
        
        self._data[new_hash] = item       
...
class QuadraticHashTable():
    ...
    def store(self, item):
        ...
        i = 0
        while self._data[new_hash]:
            new_hash = self._next_free_slot(new_hash)
...
def spellcheck_with_list(document_word_list, dictionary_word_list, quiet_mode=False):
    ...
    for doc_word in document_word_list:
        if doc_word not in dictionary_word_list:
            print(doc_word)
            unique_errors.add(doc_word)            
            num_errors += 1
