...
def hash_result_finder(tested, quarantined, load_factor=0.5):
    ...
    # If the length of tested is 0 then add all the people in quarantined to the
    # results list in the form (Name, None, None)
    if len(tested) < 1:
        for quar in quarantined:
            results.append((quar[0], None, None))
    else:
        # Adds all values in the tested list to a hash table then searches for all
        # the names in quaratined within the hash table. If found then add a tuple 
        # to the results list of the form (Name, NHI, Result) if not found then
        # adds (Name, None, None) to the results list.
        
        for test in tested:
            hash_table.store_pair(test[1], (test[0], test[2]))
        for quar in quarantined:
            values = hash_table.get_value(quar)
            if values:
                results.append((quar, values[0], values[1]))
            else:
                results.append((quar, None, None))
